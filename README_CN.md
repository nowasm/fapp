...
React/HTML ──web2canvas───────┤── canvas.json ─▶ figmalib::Document ─┬─▶ ThorVG 光栅化 ─▶ 引擎纹理
       （真实浏览器渲染后采集）  │   （自动识别格式）   (Frame/Text/…)   │    （raylib / Unity / 自研运行时…）
                                                                      └─▶ fapp2godot ─▶ Godot 4 工程
                                                                              (.tscn + PNG sprites)
```
...
apps/fapp2godot/    fapp2godot — canvas.json → Godot 4 工程（.tscn + sprites）
...
## React/HTML → Godot 工程（web2canvas + fapp2godot）
...
该工具只链接 figmalib 核心静态库（不需要 raylib/quickjs），可在精简的
`build_godot` 目录里秒级编出。
...
跑通 web2canvas → fapp2godot，中间产物落在 `<godotDir>/.web2canvas/`，`<godotDir>/`
