...
喂给 AI，让它通过 figmaedit MCP 建出来的设计有审美下限，而不是每次从零拍脑袋。
...
TOKEN_MAPPING.md              # design-tokens.json -> figmaedit 调用的映射规则 ★必读
...
3. **建节点**：按 [`TOKEN_MAPPING.md`](TOKEN_MAPPING.md) 把 token 折算成 figmaedit
   的 `create_node` / `update_nodes` 属性（填充色、圆角、文本样式、阴影）。
4. **审计**：`figmaplay <app> --shot` 出截图后，按 `skills/design-review` 对着这套
